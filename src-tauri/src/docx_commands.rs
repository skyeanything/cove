...
/// 将 DOCX data-URL 通过系统 Pages 静默转换为 PDF data-URL。
/// 使用 spawn_blocking 在 Tokio 线程池执行，IPC 主线程始终响应。
#[tauri::command]
pub async fn docx_to_pdf_via_pages(
    app: tauri::AppHandle,
    data_url: String,
) -> Result<String, String> {
    let office_app = find_office_app(&["Pages"])
        .ok_or_else(|| "未找到 Pages，请从 App Store 安装 macOS Pages".to_string())?;

    tauri::async_runtime::spawn_blocking(move || {
        convert_to_pdf(app, data_url, "docx", office_app)
    })
    .await
