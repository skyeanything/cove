...
| 参数 | 类型 | 说明 |
|------|------|------|
| `command` | `string` | Cove wrapper 命令或 OfficeLLM 文档命令 |
| `args` | `Record<string, string>?` | 命令参数，统一使用键值对 object |

**Wrapper 命令**

| command | 说明 |
|--------|------|
| `help` | 查看 bundled office 的命令发现说明；`args.name` 可查看单个 wrapper 或文档命令 |
| `detect` | 检测本地是否已安装 office sidecar |
| `doctor` | 检查外部依赖状态（LibreOffice、poppler 等） |
| `list-commands` | 列出 OfficeLLM 运行时支持的文档命令，可选 `args.category` 过滤 |
| `get-command-schema` | 查看单个 OfficeLLM 文档命令的真实 schema；需要 `args.name` |
...
```json
{ "command": "list-commands", "args": { "category": "Editing" } }
```

```json
{ "command": "get-command-schema", "args": { "name": "replace-text" } }
```

```json
{ "command": "open", "args": { "path": "report.docx" } }
```

```json
{ "command": "replace-text", "args": { "find": "Draft", "replace": "Final" } }
```

```json
{ "command": "save", "args": { "path": "report-final.docx" } }
```

> **注意**：此工具调用的是内嵌 sidecar，与外部安装的 CLI 是独立的两套体系。
> 对不确定的文档命令，先执行 `help` / `list-commands` / `get-command-schema`，不要直接猜命令名。
